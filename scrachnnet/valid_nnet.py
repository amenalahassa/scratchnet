...
def main():
    ...
    losses, train_labels, train_predicted, test_predicted, prediction_time = mlp.train(X_train, y_train, X_test, loss_function, learning_rate=learning_rate, epochs=epochs)

    # Pytorch
    pt_model  = mlp.as_pytorch()
    pt_losses, pt_train_labels, pt_train_predicted, pt_test_predicted, pt_prediction_time = mlp.pytorch_train(pt_model, X_train, y_train, X_test, loss_function, learning_rate=learning_rate, epochs=epochs)
    ...
    # Plot losses
    helpers.plot(range(epochs), losses, title="Loss - Train")
    helpers.plot(range(epochs), pt_losses, title="Loss - Pytorch Train")

    # print("Time taken for prediction:", prediction_time)
